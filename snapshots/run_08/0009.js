...
class Agent {
  ...
  updatePosition(others) {
    ...
    // TENSILE STRENGTH: Draw lines directly to the buffer to create sharp structural fibers
    // that form the backbone of the micro-web.
    const xf = floor(this.x);
    const yf = floor(this.y);
    const index = (xf + yf * width) * 4;
    
    const baseDeposit = (22 + (currentDensity * 95)) * (0.5 + pulse);
    const r = min(pixels[index] + baseDeposit * (0.35 + currentDensity * 0.6), 255);
    const g = min(pixels[index+1] + baseDeposit * (1.3 - currentDensity * 0.3), 255);
    const b = min(pixels[index+2] + baseDeposit * (2.0 - currentDensity * 1.0), 255);

    pixels[index] = r;
    pixels[index + 1] = g;
    pixels[index + 2] = b;

    // Draw a bridging tendon if moving at high velocity (low density regions), reinforcing the architecture
