...
function setup() {
  ...
  for(let i=0; i<6; i++){
    nucleationPoints.push({
      x: width/2 + cos(i * TWO_PI / 6) * 300,
      y: height/2 + sin(i * TWO_PI / 6) * 300
    });
  }
}
...
class Agent {
  ...
  updateDirection() {
    ...
    // CRYSTALLINE NUCLEATION: Agents are weakly attracted to fixed nucleation points,
    // causing the microstructure to transition from a single central blob to a distributed, 
    // crystalline network of interconnected mineralized colonies.
    let pullX = 0;
    let pullY = 0;
    ...
    if (abs(pullX) > 0.01) {
      ...
      this.dir = lerp(this.dir, targetDir, 0.02);
    }
  ...
  updatePosition() {
    ...
    const currentDensity = this.sense(0, 0) / 255.0;
    const speed = lerp(2.2, 0.5, currentDensity);
    ...
    pixels[index] = min(pixels[index] + deposit * 0.7, 255);
    pixels[index + 1] = min(pixels[index+1] + deposit * 1.0, 255);
    pixels[index + 2] = min(pixels[index+2] + deposit * 1.6, 255);
  }
}
