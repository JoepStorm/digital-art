  ...
  updatePosition(others) {
    ...
    if (random() < 0.08) {
      ...
      if (d2 < 3000 && d2 > 16) { 
        ...
        // THE CHITIN-ARCHITECT: Polymerizing chitinous membranes between nearby agents in structural zones
    ...
    // THE HYPHAE-GARDENER: Fruiting bodies emerge in dense, structural nexus points
    if (currentDensity > 0.85 && frameCount % 60 === 0 && random() < 0.1) {
      noStroke();
      // Cyan to Teal bioluminescent spores
      fill(0, 220, 200, 40); 
      for(let j=0; j<5; j++) {
