...
function draw() {
  ...
  // Slowly shifting deep-ocean background gradient
  // The gradient breathes with population dynamics — more predators = warmer deeper tones
  const breathPhase = frameCount * 0.005;
  const predatorPressure = constrain(predatorAgents.length / 1500, 0, 1);
  
  // Top color: deep midnight blue, shifts toward indigo with predator pressure
  const topR = 2 + predatorPressure * 8;
  const topG = 3 + sin(breathPhase) * 2;
  const topB = 12 + sin(breathPhase * 0.7) * 4;
  
  // Bottom color: abyssal blue-black, shifts toward deep crimson with predator pressure
  const botR = 4 + predatorPressure * 15;
  const botG = 1;
  const botB = 6 + (1 - predatorPressure) * 6;
  
  // Render trails to canvas pixels with tone mapping and deep ocean background gradient
  loadPixels();
  for (let y = 0; y < h; y++) {
    // Vertical gradient interpolation factor
    const yFrac = y / h;
    // Non-linear gradient for more depth in the lower portion
    ...
    for (let x = 0; x < w; x++) {
      ...
      // Blend: use additive over the background gradient
      // Trail brightness determines how much background shows through
      pixels[i]     = min(255, bgR + trR);
      pixels[i + 1] = min(255, bgG + trG);
      pixels[i + 2] = min(255, bgB + trB);
      pixels[i + 3] = 255;
    }
  }
