...
function depositTrail(px, py, r, g, b, intensity) {
  ...
  // Scale deposit amount much higher so trails actually accumulate visibly
...
function draw() {
  ...
  const w = width;
  const h = height;
  const decayRate = 0.965; // slower decay for more persistent tendrils
  const diffuseWeight = 0.3; // stronger diffusion for smoother organic shapes
