...
class PreyAgent {
  ...
  sense(angleOffset) {
    ...
    // Attracted to own trail (green channel for cyan = g+b), repelled by red (predator trail)
    return trailA[idx + 1] + trailA[idx + 2]; // cyan attraction
  }
  ...
  senseDanger(angleOffset) {
    ...
    const idx = (sx + sy * width) * 3;
    return trailA[idx]; // red channel = predator presence
  }
  
  update() {
    // Physarum-style sensing for trail following
    ...
    // Flee from predator trails — steer away from red channel
    ...
    if (dangerTotal > 0.5) {
      // Steer away from the direction with most danger
    ...
    const speed = preySpeed;
    depositTrail(this.x, this.y, preyColor[0], preyColor[1], preyColor[2], speed);
  }
}
...
class PredatorAgent {
  ...
  sense(angleOffset) {
    ...
    const idx = (sx + sy * width) * 3;
    // Attracted to prey trails (green+blue for cyan), repelled by own red trails
    const preySignal = trailA[idx + 1] + trailA[idx + 2];
    const selfSignal = trailA[idx]; // red = own trail
  ...
  update() {
    ...
    // Apply flow field (predators are slightly less affected)
    const t = frameCount * flowTimeSpeed;
    const flowA = getFlowAngle(this.x, this.y, t);
    ...
    const speed = predatorSpeed;
    depositTrail(this.x, this.y, predatorColor[0], predatorColor[1], predatorColor[2], speed);
  }
}
