...
const synapseSampleCount = 200; // max predators to check for synapses
...
function drawSynapseNetwork() {
  ...
  // Sample a subset of predators for performance
  const sampled = [];
  ...
  // Sort by x for spatial locality optimization
  ...
  for (let i = 0; i < sampled.length; i++) {
    ...
    for (let j = i + 1; j < sampled.length; j++) {
      const b = sampled[j];
      const dx = b.x - a.x;
      if (dx > synapseMaxDist) break; // sorted by x, so no further matches
      ...
      // Flickering based on noise for organic feel
      ...
      // Color shifts from white-magenta to electric blue based on speed
      const speedA = a.speed || predatorSpeed;
  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++) {
      ...
      if (trailBright > 2.0) {
        ...
        const preyness = tg / max(1, tr + tg + tb);
        const predness = tr / max(1, tr + tg + tb);
        
        // Prey trails shift through cool hues, predator trails through warm hues
        const spatialHue = noise(x * 0.002, y * 0.002, hueShiftTime) * 0.3;
        
        const preyHue = 0.5 + spatialHue + hueShiftTime * 3;
        const predHue =
