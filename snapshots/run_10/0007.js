...
// Predator agents: chase prey by following green/blue trails, but repel from own red trails
// This self-repulsion prevents clumping and creates sweeping hunting formations
class PredatorAgent {
  constructor() {
    this.x = random(width);
    this.y = random(height);
  ...
  // Sense prey trails (green/blue) and own trails (red) at the sensor position
  sense(dirOffset) {
    const angle = this.dir + dirOffset;
    ...
    y = (y + height) % height;
    const index = (x + y * width) * 4;
    let redVal = pixels[index];        // own trail - repel
    let greenVal = pixels[index + 1];  // prey trail - attract
    let blueVal = pixels[index + 2];   // prey trail - attract
    // Chase prey trails but avoid areas already saturated with own predator trails
