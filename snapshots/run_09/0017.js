...
function draw() {
  ...
  // Quantum Gravitational Lensing: Periodic distortions that act as lenses over high-activity areas
  if (frameCount % 2 === 0) {
    let warpStr = map(sin(frameCount * 0.01), -1, 1, 1, 6);
    // Modified copy to create a slight radial bulge effect
    copy(this, warpStr, warpStr, width - warpStr * 2, height - warpStr * 2, 0, 0, width, height);
    
    // Create 'lensing' rings at random high-density centers
    if (frameCount % 60 === 0) {
