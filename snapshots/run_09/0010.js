...
function setup() {
  ...
  background(0);
  agents = new Agents();
  // Structures buffer to hold the 'calcified' geometry
...
function draw() {
  ...
  // Blend structures into the main canvas as "calcified" obstacles/features
  ...
  // Vitruvian Calcification: Slowly age the static structures
...
class Agent {
  ...
  updateDirection() {
    ...
    // Agents react strongly to the 'calcified' bone-white structures
    if (center > 650) {
      this.dir += PI;
      // Crystallization: When agent hits a high-density zone, draw a line in the structure buffer
      if (random() < 0.05) {
