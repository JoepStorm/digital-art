...
function draw() {
  ...
  structures.background(0, 0, 0, 8); // Slightly faster fade for the new electrical arcs
...
class Agents {
  ...
  update() {
    ...
    for (let i = this.list.length - 1; i >= 0; i--) {
      ...
      // Plasma Synchrony: Spatial partitioning to find nearby neighbors for electric arcing
    ...
    // Effect: Draw flickering plasma arcs between nearby agents of the same type
    if (frameCount % 3 === 0) {
      structures.push();
      structures.strokeWeight(0.5);
      for (let [key, cellAgents] of spatialGrid) {
        if (cellAgents.length > 2) {
          for (let i = 0; i < min(cellAgents.length, 3); i++) {
            let a1 = cellAgents[i];
            let a2 = cellAgents[floor(random(cellAgents.length))];
            if (a1 !== a2 && a1.type === a2.type) {
              let arcAlpha = map(sin(frameCount * 0.2 + i), -1, 1, 20, 150);
