...
// Iteration 19: The Weaver - Introduced gravitational warping where mouse proximity bends spacetime and pulls local agents into orbits
// Iteration 20: The Neural Weaver - Introduced chromatic aberration and trail bleeding to create depth-indexed spectral blurring
const agentsNum = 400;
...
function draw() {
  ...
  // Custom decay with crystallization, gravitational lensing, and spectral bleeding
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (r > 20) {
          ...
          // Iteration 20: Chromatic Bleeding. Trails slightly "leak" color into neighbors
          // to create a painterly, organic feel
          if (random() < 0.1) {
              let nx = (x + floor(random(-1, 2)) + width) % width;
              let ny = (y + floor(random(-1, 2)) + height) % height;
              let ni = (nx + ny * width) * 4;
              pixels[ni] = (pixels[ni] + r * 0.1) % 255;
              pixels[ni+1] = (pixels[ni+1] + g * 0.1) % 255;
              pixels[ni+2] = (pixels[ni+2] + b * 0.1) % 255;
          }
      } else if (r > 0) {
          pixels[i] *= 0.995;
      }
...
class Agent {
  ...
  updateDirection() {
    ...
    // Gravitational anchor logic
    let angleToCenter = atan2(height / 2 - this.y, width / 2 - this.x);
    let diff = angleToCenter - this.dir;
    while (diff < -PI) diff += TWO_PI;
    while (diff > PI) diff -= TWO_PI;
    
    let dMouse = dist(this.x, this.y, mouseX, mouseY);
    if (dMouse < 200) {
        let angleToMouse = atan2(mouseY - this.y, mouseX - this.x);
        let pull = map(dMouse, 0, 200, 0.1, 0);
        this.dir = lerp(this.dir, angleToMouse + HALF_PI, pull);
    }
