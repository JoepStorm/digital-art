...
class Agent {
  ...
  updatePosition() {
    ...
    // Decay mechanism: agents are more likely to die in extremely crowded areas (entropy)
    // This prevents the screen from becoming a single solid blob of light
    if (pixels[index] > 240 && random() < 0.06) {
        this.dead = true;
        return;
    }
...
class Agents {
  ...
  update() {
    // Process movement and direction
