...
class Agent {
  ...
  updatePosition() {
    ...
    let h = (d * 0.2 + this.dir * 30 + frameCount * 0.5) % 255;
    let s = map(this.energy, 0.2, 1.0, 100, 255);
    let b = map(sin(frameCount * 0.05), -1, 1, 180, 255);
    
    let c = color(h, s, b);
    
    // Drawing with slight "jitter" to create organic thickness
    for(let ox = -1; ox <= 1; ox++) {
      for(let oy = -1; oy <= 1; oy++) {
        if(random() > 0.6) {
          let pIdx = (((ix + ox + width) % width) + ((iy + oy + height) % height) * width) * 4;
          pixels[pIdx] = red(c);
          pixels[pIdx + 1] = green(c);
          pixels[pIdx + 2] = blue(c);
          pixels[pIdx + 3] = 255;
        }
      }
    }
