...
function initSim() {
  ...
  particles = Array.from({ length: numParticles }, () => ({
    ...
    neighbors: 0,
    cliques: 0,
    metabolism: 0 // New potential for heat/radiation
  }));
}
...
function draw() {
  ...
  for (let a of particles) {
    ...
    // Metabolic calculation: energy builds up based on connectivity and complexity
  ...
  for (let p of particles) {
    ...
    // Metabolic radiation: Highly active hubs emit ghost-like structural echoes
    if (metabolicToggle.checked() && p.metabolism > 12) {
      ...
      for(let r = 0; r < ringCount; r++) {
        ...
        strokeWeight(0.5);
        // Radiate using the shape determined by 'cliques' - circular if solo, angular if architectural
        if (p.cliques > 5) {
          rectMode(CENTER);
