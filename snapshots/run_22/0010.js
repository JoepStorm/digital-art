...
// Iteration 9: The Fractal Weaver - Connected sub-graphs now generate recursive shadow geometries that reveal their internal topological depth
// Iteration 10: The Chronos Weaver - Temporally shifted graph ghosts that visualize the recent kinetic history of structures
const numTypes = 5;
...
function setup() {
  ...
  fractalToggle = createCheckbox('Fractal Topology', true).parent(content);
  chronosToggle = createCheckbox('Temporal Chronos (Ghosts)', true).parent(content);
...
function initSim() {
  ...
  particles = Array.from({ length: numParticles }, () => ({
    ...
    metabolism: 0,
    resonance: 0,
    history: [] // Added for Chronos Weaver
  }));
}
...
function draw() {
  ...
  for (let a of particles) {
    // Record history for temporal effects
    if (chronosToggle.checked()) {
      ...
      if (d2 > 0 && d2 < rMax * rMax) {
        ...
        if (a.type === b.type) {
            ...
            if (connectivityToggle.checked() && d < rMax * 0.7 && abs(b.x - a.x) < width/2 && abs(b.y - a.y) < height/2) {
              ...
              // Recursive Shadow Geometry: Add fractal layers connecting midpoints
              if (fractalToggle.checked() && d > 10) {
                drawFractalEdge(a.x, a.y, b.x, b.y, 2, colors[a.type]);
              }

              // Temporal Graphs: Connect past versions of nodes to visualize motion flow
              if (chronosToggle.checked() && a.history.length > 10 && b.history.length > 10) {
                let ghostIdx = 5;
                let ha = a.history[a.history.length - 1 - ghostIdx];
                let hb = b.history[b.history.length - 1 - ghostIdx];
                if (dist(ha.x, ha.y, hb.x, hb.y) < d * 1.2 && abs(ha.x - hb.x) < width/2) {
...
function drawFractalEdge(x1, y1, x2, y2, depth, col) {
  if (depth <= 0) return;
  ...
  let offset = (sin(frameCount * 0.04 + depth) * dist(x1, y1, x2, y2)) * 0.2;
  let px = mx - dy * (offset / dist(x1, y1, x2, y2));
  let py = my + dx * (offset / dist(x1, y1, x2, y2));
  ...
  drawFractalEdge(x1, y1, px, py, depth - 1, col);
  drawFractalEdge(px, py, x2, y2, depth - 1, col);
}
