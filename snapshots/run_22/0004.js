...
function initSim() {
  ...
  particles = Array.from({ length: numParticles }, () => ({
    ...
    neighbors: 0,
    cliques: 0 // Track structural complexity
  }));
}
...
function draw() {
  ...
  for (let a of particles) {
    ...
    for (let b of particles) {
      ...
      if (d2 > 0 && d2 < rMax * rMax) {
        ...
        if (a.type === b.type) {
            neighborCount++;
            // Architectural check: if neighbors are also connected to each other, complexity rises
  ...
  for (let p of particles) {
    ...
    // Architectural effect: High complexity nodes grow rectangular 'foundation' halos
