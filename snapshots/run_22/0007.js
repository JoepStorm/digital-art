...
function draw() {
  ...
  // Crystalline Weaving: Clusters with high neighbor count connect to distant nodes in their hierarchy
  if (crystallineToggle.checked()) {
    noFill();
    for (let i = 0; i < particles.length; i++) {
      let p = particles[i];
      if (p.neighbors > 12) {
        // Find a "distant relative" in the particle list of the same type
        ...
        if (target.type === p.type && target !== p) {
           ...
           // Only draw if within a reasonable 'crystalline' range and not crossing border
           if (d < 250 && abs(target.x - p.x) < width/2 && abs(target.y - p.y) < height/2) {
             stroke(colors[p.type] + "22");
             strokeWeight(0.3);
             // Draw a geometric 'crystalline' scaffold
