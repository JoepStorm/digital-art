...
function draw() {
  ...
  // Selective Decay & Glacial Drift:
  // We process the pixels backwards to create a subtle "downward" settling effect (Glacial Drift)
  // while applying the ossified decay logic.
  for (let y = height - 1; y >= 0; y--) {
    for (let x = 0; x < width; x++) {
      ...
      if (brightness < 255) {
        // Glacial Drift: Dense ink (dark) has a small chance to "drift" downwards, 
        // simulating the melting or sagging of heavy structures.
        if (y < height - 1 && brightness < 150 && random() < 0.02) {
          let below = (x + (y + 1) * width) * 4;
          // Swap a bit of the darkness downwards
