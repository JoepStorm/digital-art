...
class Agent {
  ...
  updateDirection() {
    ...
    // High-frequency jitters layered on a slow breathing wave create a more organic search pattern
    const phase = frameCount * 0.03 + (this.x * 0.001) + (this.y * 0.001);
    const pulseStrength = Math.pow(sin(phase), 4); // Steep pulses rather than smooth waves
