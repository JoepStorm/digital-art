...
function draw() {
  ...
  // Selective Decay & Ossification:
  // The environment "heals" fast for thin trails, but slows down for deep ink (dark colors).
  for (let i = 0; i < pixels.length; i += 4) {
    let brightness = pixels[i];
    if (brightness < 255) {
      let decayRate = brightness > 200 ? 3 : (brightness < 40 ? 0.3 : 1);
...
class Agent {
  ...
  updateDirection() {
    ...
    // Tenebrous Echo Change: 
    // Agents used to prefer dark spots (lower sensor value). 
    // Now we introduce a bias where if the center is TOO dark (ossified), 
    // they are repelled from it, preventing them from looping in the same spot forever.
    let centerWeight = center;
    if (center < 50) centerWeight += 120; // Repulsion from deep ink
  ...
  updatePosition() {
    ...
    // Liminal Threshold: Agents lose their ability to leave a mark as they reach the edge of the world.
    const borderDist = min(min(this.x, width - this.x), min(this.y, height - this.y));
    const presence = constrain(borderDist / 100, 0, 1);
    
    // Final ink deposition accounts for age (vitality) and position (presence)
