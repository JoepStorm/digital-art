...
function draw() {
  ...
  // This allows the densest "skeletons" of the collective to linger longer against the void.
  for (let i = 0; i < pixels.length; i += 4) {
    let brightness = pixels[i];
    if (brightness < 255) {
      // If brightness is very low (dense trail), decay is much slower.
      // 0-50: skeleton (very slow), 51-200: body (normal), 200+: ghost (aggressive)
