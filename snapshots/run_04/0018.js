  ...
  for (let y = height - 1; y >= 0; y--) {
    for (let x = 0; x < width; x++) {
      ...
      if (brightness < 255) {
        ...
        // If a patch is very dark, it has a tiny chance to fracture, pushing its darkness outward
        // in a non-linear way, creating thin capillaries across the void.
        if (brightness < 80 && random() < 0.0008) {
           ...
           if(tx >= 0 && tx < width && ty >= 0 && ty < height) {
             ...
             // Leave a light "scar" where the ink was pulled from
        ...
        pixels[i]   = min(255, pixels[i]   + decayRate * 0.85); // Red stays longer
        pixels[i+1] = min(255, pixels[i+1] + decayRate * 0.95); // Green stays medium
        pixels[i+2] = min(255, pixels[i+2] + decayRate * 1.20); // Blue vanishes fast
      }
    }
  }
...
class Agent {
  ...
  updateDirection() {
    ...
    // Pulmonary Mirage: A rhythmic, non-linear pulsing of the sensing range
    ...
    // Tenebrous Echo: Repulsion from deep ink
  ...
  updatePosition() {
    ...
    // Fractured Memories: Occasional mirroring of position to create ghostly symmetry
    if (random() < 0.0005) {
      this.x = width - this.x;
