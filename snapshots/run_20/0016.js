...
// Iteration 15: The Stellar-Oracle - Introducing geometric halo resonances that manifest as crystalline auras around dense star clusters
// Iteration 16: The Celestial Scribe - Introducing rhythmic orbital calligraphy through velocity-aligned glyph trails
const numTypes = 5;
...
function draw() {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    // The Celestial Scribe: Drawing glyph-like trails aligned with velocity vectors
    // This adds a sense of "cosmic writing" to the movement of the stars.
    if (abs(a.vx) > 0.1 || abs(a.vy) > 0.1) {
      push();
      translate(a.x, a.y);
      rotate(atan2(a.vy, a.vx));
      noFill();
      stroke(255, 255, 255, 15);
      strokeWeight(0.3);
      let scribeLen = (abs(a.vx) + abs(a.vy)) * 5;
      // Drawing small arcs that suggest orbital curvature
      arc(-scribeLen, 0, scribeLen * 2, scribeLen, PI, TWO_PI);
      pop();
    }
    
    // Geometric Halo Resonances - Drawing crystalline rings around dense nodes
    if (density > 4 && i % 3 === 0) {
      noFill();
      let haloSize = 15 + sin(phase + a.pulseOffset) * 5;
      stroke(220, 230, 255, 8);
      strokeWeight(0.5);
      // Drawing a subtle hexagonal aura for nodes that form the core of constellations
      beginShape();
      for (let s = 0; s < 6; s++) {
        let ang = s * PI / 3 + phase * 0.2;
        vertex(a.x + cos(ang) * haloSize, a.y + sin(ang) * haloSize);
      }
      endShape(CLOSE);
    }
    ...
    for(let n of neighbors) {
        ...
        if (abs(a.x - nX) < width/2 && abs(a.y - nY) < height/2) {
            ...
            strokeWeight(0.2 + density * 0.04);
            stroke(255, 60, 40, 18 * distAlpha);
            line(a.x - a.vx, a.y - a.vy, nX - a.vx, nY - a.vy);
            stroke(40, 120, 255, 18 * distAlpha);
            line(a.x + a.vx, a.y + a.vy, nX + a.vx, nY + a.vy);
            
            let strokeCol = color(colors[a.type]);
            strokeCol.setAlpha(60 * distAlpha + (density * 2));
            stroke(strokeCol);
            line(a.x, a.y, nX, nY);
        }
    }
