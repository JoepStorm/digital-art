...
function draw() {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    for(let n of neighbors) {
        ...
        if (abs(a.x - nX) < width/2 && abs(a.y - nY) < height/2) {
            ...
            // Iteration 13: Astral Silk - weave fine cross-connections between neighbors of a common node
            // This creates a "scaffold" look within constellations
            if (density > 3 && frameCount % 2 === 0) {
              for (let k = 0; k < neighbors.length; k++) {
                 ...
                 if (dBetween < 30) {
                    ...
                    strokeWeight(0.2);
                    line(nX, nY, a.x + other.dx, a.y + other.dy);
                 }
              }
            }
  ...
  for (let p of particles) {
    ...
    noStroke();
    fill(255, 250, 243, 180 + twinkle * 75);
    circle(p.x, p.y, 1.2);
    
    let glow = color(colors[p.type]);
    glow.setAlpha(15); 
    fill(glow); 
    circle(p.x, p.y, 6 + twinkle * 6);
  }
}
