...
function draw() {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    for (let j = 0; j < particles.length; j++) {
      ...
      // Wrap-around distance logic to prevent lines snapping across the screen
      ...
      if (d2 < rMax * rMax) {
        ...
        // Physics logic: The Star-Charter's Angular Bias
        // We calculate the angle to influence particles to snap into 60-degree increments (hexagonal/linear)
        ...
        if (d < 22) {
           ...
           // Apply a small perpendicular force to align stars into "charts"
        ...
        if (d < 40) {
          neighbors.push({d: d, x: a.x + dx, y: a.y + dy});
        }
      }
    }
    
    // Draw connections to nearby stars to visualize the emerging constellation chart
    // Limited to local neighbors to maintain a clean, minimalist aesthetic
    for(let n of neighbors) {
      strokeWeight(0.6);
      let strokeCol = color(colors[a.type]);
      strokeCol.setAlpha(80 * (1 - n.d / 40));
      stroke(strokeCol);
      // Only draw if within bounds to avoid screen-wrap flashes
  ...
  for (let p of particles) {
    ...
    c.setAlpha(20 + twinkle * 30);
    fill(c);
    circle(p.x, p.y, 4 + twinkle * 4);
  }
}
