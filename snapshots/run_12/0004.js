  ...
  interact(boids) {
    ...
    for (let i = boids.length - 1; i >= 0; i--) {
      ...
      if (d < perception) {
        ...
        // Gravitational Pull: Large boids exert a vortex-like attraction on smaller ones
        if (this.mass > o.mass * 2) {
            let orbit = p5.Vector.sub(o.pos, this.pos);
            orbit.rotate(HALF_PI); // Create a swirling tangent force
...
class Flock {
  ...
  draw() {
    this.boids.forEach(b => {
      ...
      // Draw body as a dynamic kite-like shape pointing towards movement
      push();
