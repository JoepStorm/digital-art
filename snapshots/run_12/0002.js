...
class Flock {
  ...
  draw() {
    this.boids.forEach(b => {
      // Draw a luminous trail using the boid's history
      noFill();
      strokeWeight(b.mass * 0.5);
      beginShape();
      for(let i = 0; i < b.history.length; i++) {
        let alpha = map(i, 0, b.history.length, 0, 255);
        stroke(b.hue, 150, 255, alpha);
        vertex(b.history[i].x, b.history[i].y);
      }
      endShape();
