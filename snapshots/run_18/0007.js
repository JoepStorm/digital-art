...
// Iteration 7: The Polyphonist - Introduced synchronized orbital rotation around the harmonic center to create a sense of celestial choreography
const numTypes = 5;
...
function draw() {
  ...
  for (let a of particles) {
    ...
    // Polyphonic Rotation: A tangential force that scales with distance, 
    // creating a harmonious whirlpool effect around the center.
    let rotationStr = 0.00015 * (1 + cos(pulsePhase * 0.5));
    fx += (-gdy / (gd + 1)) * rotationStr * gd;
    fy += (gdx / (gd + 1)) * rotationStr * gd;
