...
class Agent {
  constructor(id, parentX, parentY, parentDir, species) {
    ...
    // Iteration 11: Tethering state to simulate anchoring to high-density trails
  ...
  updatePosition() {
    ...
    // Iteration 11: Nematomorph Anchoring
    // Agents have a chance to "stick" to areas of very high trail density, pulling 
    // their movement into a tight orbit or "knot" until their phase shifts.
    ...
    if (this.isAnchored) {
      // Pull back towards anchor, creating a swirl or "knot"
      let adx = this.anchorX - this.x;
      let ady = this.anchorY - this.y;
      moveX += adx * 0.2;
      moveY += ady * 0.2;
      if (sin(this.phase) < -0.9) this.isAnchored = false; // Release anchor when phase is low
    }
    ...
    this.x += moveX * (this.speed * velocityMultiplier);
    this.y += moveY * (this.speed * velocityMultiplier);
