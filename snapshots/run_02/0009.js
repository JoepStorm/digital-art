  ...
  updateInteraction(others, spawnArray) {
    ...
    if (d2 < 625) { 
      if (neighbor.species !== this.species) {
        ...
        // When two high-energy opposing species meet, they have a chance to spawn a visual filament branch
...
class Agents {
  ...
  update() {
    ...
    // Manage temporary branched agents
    if (spawnQueue.length > 0) {
      this.agentsList = this.agentsList.concat(spawnQueue);
    }
    // Remove dead clones to keep performance stable
