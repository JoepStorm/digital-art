...
function draw() {
  // Increased opacity of background fade to create high-contrast "voids" between filaments
  background(2, 5, 12, 22); 
  ...
  for (let i = 0; i < pixels.length; i += 4) {
    ...
    if (bVal > 0.1) {
      ...
      let crystal = bVal > 180 ? 1.6 : 1.0;
      let echo = bVal > 40 ? 0.8 : 0.4;
      pixels[i]     = min(255, pixels[i]     + bVal * 0.25 * crystal * echo);
      pixels[i + 1] = min(255, pixels[i + 1] + bVal * 0.12 * crystal * echo);
      pixels[i + 2] = min(255, pixels[i + 2] + bVal * 0.55 * crystal * echo);
    }
  }
...
class Agent {
  ...
  updatePosition() {
    ...
    // Spatial Gravity: Agents are pulled towards intense light sources (event horizons)
    if (pixels[idx * 4] + pixels[idx * 4 + 1] + pixels[idx * 4 + 2] > 600) {
        let pull = 0.05;
        this.dir += sin(atan2(height/2 - this.y, width/2 - this.x) - this.dir) * pull;
    }
    ...
    if (this.isAnchored) {
      // Elastic pull toward anchor point
      moveX += (this.anchorX - this.x) * 0.3;
      moveY += (this.anchorY - this.y) * 0.3;
      // Release anchor based on internal rhythm
      if (sin(this.phase) < -0.9) this.isAnchored = false; 
    }

    // Viscous resistance makes dense areas slower to navigate
    ...
    // Brightness based on energetic state and oscillatory phase
    const shift = sin(this.phase) * 45;
    const boost = (this.energy - 1.0) * 85;
    
    // Write trail directly to pixel buffer with slight blending
    ...
    // Accumulate pheromones into bloomGrid for sensing and glow effect
    bloomGrid[npx + npy * width] += 60 * this.energy;
  }
}
...
class Agents {
  ...
  update() {
    let spawnQueue = [];
    // Shuffle check - only a subset interact per frame for performance
    ...
    // Management of agent count to prevent frame drops
    if (this.agentsList.length > 7000) {
        this.agentsList.splice(0, this.agentsList.length - 7000);
    }
  }
}
