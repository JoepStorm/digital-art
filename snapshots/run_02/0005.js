...
class Agent {
  ...
  updateDirection() {
    ...
    // Dynamic obstacle/density avoidance (legacy logic integrated into affinity)
    if (this.repulse) {
      const threshold = 400; 
  ...
  // The sensing logic now weighs different color channels based on species affinity
  // Red = Species 0 (Pink), Green = Species 1 (Cyan), Blue = Species 2 (Blue)
  sense(dirOffset, dist) {
    ...
    // Calculate signal by weighting the RGB channels by this agent's specific affinities
    // This allows species to "stalk" or "avoid" the trails of other species
