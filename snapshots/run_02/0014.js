...
class Agent {
  constructor(id, parentX, parentY, parentDir, species) {
    ...
    if (this.species === 0) {
      ...
      this.charge = 1.0; // Positive charge
      ...
      this.charge = -1.0; // Negative charge
    } else {
      this.baseColor = [80, 100, 255]; 
      this.baseSensorOffset = 42;
      this.speed = 1.7;
      this.repulse = false;
      this.pulseFreq = 0.025;
      this.affinity = [-0.3, 0.7, 1.2];
      this.charge = 0.5; // Weak positive
    }
  }
  ...
  updateInteraction(others, spawnArray) {
    ...
    if (d2 < 900) { // Increased interaction radius
      // Magnetic steering: Use cross product of heading and relative vector to simulate Lorentz force
      let angleToNeighbor = atan2(dy, dx);
      let relativeAngle = angleToNeighbor - this.dir;
      let force = (this.charge * neighbor.charge) * sin(relativeAngle) * 0.15;
      this.dir += force;
