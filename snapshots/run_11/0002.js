...
function draw() {
  // A slightly more persistent background for longer trails
  background(0, 15);
  flock.update();
  flock.draw();
}
    ...
    for (let o of boids) {
      ...
      if (o !== this && d < perception) {
        if (o.species === this.species) {
          ...
          // This creates "territorial" pockets and more complex fluid dynamics
...
class Flock {
  ...
  draw() {
    this.boids.forEach(b => {
      ...
      let alpha = map(pulse, -1, 1, 100, 255);
      ...
      let length = map(pulse, -1, 1, 10, 20);
