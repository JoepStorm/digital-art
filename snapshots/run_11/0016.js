...
function setup() {
  ...
  blendMode(SCREEN);
  flock = new Flock();
  // Spores act as stationary architectural anchors
  for (let i = 0; i < 12; i++) {
    spores.push({
  ...
  // Moons are dynamic singular points of high localized attraction for each species
  for (let i = 0; i < 3; i++) {
    moons.push({
      pos: createVector(random(width), random(height)),
...
function draw() {
  ...
  // Update and render moons: moving singularities that pull species into tight orbits
  moons.forEach(m => {
    m.pos.add(m.vel);
    ...
    // Draw a subtle 'gravity well' line from moon center to its species representative
...
class Boid {
  ...
  steer(boids) {
    ...
    for (let o of boids) {
      ...
      if (o !== this && d < perception) {
        if (o.species === this.species) {
          ...
          // Prism refraction logic: Slow agents act as refractive nodes for other species
    ...
    // Species-specific Moon attraction: creates swirling vortexes
    ...
    if(dToMoon < 400) {
      ...
      // Tangential component creates the swirl
...
class Flock {
  ...
  draw() {
    ...
    this.boids.forEach(b => {
      ...
      // New: Chromatic filaments that bridge between high-speed boids of the same species
