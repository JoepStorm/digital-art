...
class Boid {
  constructor() {
    ...
    this.interSpeciesStress = 0; // New: Tracks proximity to other species
  }
    ...
    for (let o of boids) {
      ...
      if (o !== this && d < perception) {
        if (o.species === this.species) {
          ...
          nAvoid++;
          // Increase stress based on proximity to foreigners
          localStress += map(d, 0, perception, 1, 0);
        }
      }
    }
    
    // Smoothly update stress factor for visual effects
...
class Flock {
  ...
  draw() {
    ...
    this.boids.forEach(b => {
      ...
      // Species interaction ribbon: Throws wide glowing shadows when near different species
      if (b.interSpeciesStress > 0.2) {
        let ribbonWidth = b.interSpeciesStress * 15;
        let p1 = p5.Vector.rotate(b.vel, HALF_PI).setMag(ribbonWidth);
        let p2 = p5.Vector.rotate(b.vel, -HALF_PI).setMag(ribbonWidth);
