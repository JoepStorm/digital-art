...
class Flock {
  ...
  draw() {
    this.boids.forEach(b => {
      ...
      let alpha = map(pulse, -1, 1, 80, 220);
      let weight = map(pulse, -1, 1, 1.5, 4.5);
      
      // Iteration 5: Chromatic Aberration Halo - creates a lateral offset based on velocity direction
      // This mimics spectral light splitting behind the fast-moving path of the organisms.
      let perp = createVector(-b.vel.y, b.vel.x).setMag(3);
      
      // Offset red-heavy trail
      stroke(red(b.color), 0, 100, alpha * 0.4);
      strokeWeight(weight * 1.5);
      line(b.pos.x + perp.x, b.pos.y + perp.y, b.pos.x - b.vel.x * 4, b.pos.y - b.vel.y * 4);
      
      // Offset blue-heavy trail
      stroke(0, green(b.color), blue(b.color), alpha * 0.4);
      strokeWeight(weight * 1.5);
      line(b.pos.x - perp.x, b.pos.y - perp.y, b.pos.x - b.vel.x * 4, b.pos.y - b.vel.y * 4);
