...
function draw() {
  blendMode(BLEND);
  background(2, 10, 20, 25); // Slightly bluer tint for the void
  
  // Create a subtle, static background layer of distant "micro-stars" to increase sparkle density
  fill(255, 255, 255, 40);
  for (let i = 0; i < 50; i++) {
    let sx = noise(i, frameCount * 0.01) * width;
    let sy = noise(i + 500, frameCount * 0.01) * height;
    if (random() > 0.5) {
      circle(sx, sy, 0.8);
    }
  }
    ...
    for (let b of particles) {
      ...
      if (d2 > 0 && d2 < rMax * rMax) {
        ...
        if (a.type === b.type && d < 35) {
          stroke(colors[a.type] + "22"); // Very low opacity hex
          strokeWeight(map(d, 0, 35, 1, 0));
