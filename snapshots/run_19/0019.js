...
// Iteration 17: The Glimmering Cartographer - Adding a network of faint, glowing constellation ties between nearby particles
// Iteration 18: The Scintillating Prismatist - Adding a refraction-based ghosting effect that splits light as particles accelerate
// Iteration 19: The Scintillating Frost-Smith - Adding a layer of crystalline drift and glittering ice-dust
const numTypes = 5;
...
function draw() {
  ...
  for (let a of particles) {
    ...
    for (let b of particles) {
      ...
      if (d2 > 0 && d2 < rMax * rMax) {
        ...
        // Draw thin, ephemeral crystalline webs that pulse based on proximity
        if (a.type === b.type && d < 40) {
          let alpha = map(d, 0, 40, 45, 0) * (0.5 + 0.5 * sin(frameCount * 0.08));
          stroke(red(color(colors[a.type])), green(color(colors[a.type])), blue(color(colors[a.type])), alpha); 
          strokeWeight(0.6);
  ...
  for (let p of particles) {
    ...
    fill(255, 50, 80, baseAlpha * 0.35);
    circle(p.x - offset * 1.5, p.y - offset * 1.5, 1.8);
    fill(50, 255, 255, baseAlpha * 0.35);
    circle(p.x + offset * 1.5, p.y + offset * 1.5, 1.8);

    // Light-splitting ghosting based on velocity
    if (speed > 4.0) {
      fill(255, 255, 255, baseAlpha * 0.1);
      circle(p.x - p.vx * 2, p.y - p.vy * 2, 4);
    }
    ...
    // Glint effect - refined for "sparkling" direction
    if(random() > 0.985) {
        let starSize = random(4, 12);
        push();
        translate(p.x, p.y);
        ...
        // Added cross-glitter flare
        let dlt = starSize * 0.6;
        line(-dlt, -dlt, dlt, dlt);
        line(dlt, -dlt, -dlt, dlt);
        pop();
        noStroke();
    }
