...
function draw() {
  ...
  for (let p of particles) {
    ...
    // Added a rhythmic expansion/contraction to the offset for a "breathing" light effect
    ...
    // Radiant Diamond Flare: A more sophisticated crystalline sparkle for bright particles
    if (speed > 2.5 && random() > 0.95) {
      stroke(255, 255, 255, baseAlpha * 0.9);
      strokeWeight(0.5);
      let r = 7 + sin(frameCount * 0.3) * 3;
      // Longitudinal sparkle
      line(p.x - r, p.y, p.x + r, p.y);
      line(p.x, p.y - r, p.x, p.y + r);
      // Diagonals for high energy
      if (speed > 4.5) {
        let diag = r * 0.6;
        line(p.x - diag, p.y - diag, p.x + diag, p.y + diag);
        line(p.x + diag, p.y - diag, p.x - diag, p.y + diag);
      }
      noStroke();
    }
