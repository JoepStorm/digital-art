...
let hawkingMode = true; // Toggle for pair-production particles at "event horizons"
...
function draw() {
  ...
  for (let a of particles) {
    ...
    if (temporalMode || spinRibbonMode) {
    ...
    // Hawking Radiation: Virtual pair production when speed is high (near "horizon" limit)
    if (hawkingMode && speed > maxSpeed * 0.95 && random() < 0.05) {
      let angle = atan2(a.vy, a.vx);
      let r = 5 + sin(a.phase) * 5;
      stroke(255, 150);
      strokeWeight(1);
      // Draw a flash of a particle-antiparticle pair separating
