...
let cloudMode = true;
let warpMode = true;
let temporalMode = true; // Toggle for temporal ghosting
...
function initSim() {
  ...
  particles = Array.from({ length: numParticles }, () => ({
    ...
    phase: random(TWO_PI),
    history: [] // Added to store previous states for temporal effects
  }));
}
...
function draw() {
  ...
  for (let a of particles) {
    ...
    // Store history before movement for temporal superposition
    if (temporalMode) {
    ...
    if (tunnelingMode && random() < 0.001) {
      ...
      a.x += cos(angle) * hop;
      a.y += sin(angle) * hop;
      a.history = []; // Clear history on tunnel to show discontinuity
    }
  ...
  // Rendering Particles and their Temporal Superposition "Ghost" states
  for (let p of particles) {
    let speed = sqrt(p.vx * p.vx + p.vy * p.vy);
    
    // Draw past versions of the particle as semi-transparent temporal ghosts
