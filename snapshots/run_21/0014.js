...
// Iteration 14: The Path-Integral - Sum-over-histories manifested as temporal probability streamlines
const numTypes = 5;
...
let waveCollapseMode = true;
let pathIntegralMode = true; // New feature: Sum-over-histories streamlines
...
function setup() {
  ...
  let piToggle = createCheckbox('Path-Integral Streamlines', true);
  piToggle.position(10, 270);
  piToggle.style('color', '#ffffff');
  piToggle.changed(() => { pathIntegralMode = piToggle.checked(); });
  
  initSim();
}

function mousePressed() {
  if (mouseY > 290) initSim();
}
...
function draw() {
  ...
  for (let a of particles) {
    ...
    if (temporalMode || spinRibbonMode || waveCollapseMode || pathIntegralMode) {
      a.history.push({x: a.x, y: a.y, phase: a.phase, vx: a.vx, vy: a.vy});
  ...
  for (let p of particles) {
    ...
    // Path-Integral Streamlines: Visualizing potential alternate histories of the wave function
    if (pathIntegralMode && p.history.length > 5) {
      noFill();
      strokeWeight(0.5);
      for (let s = 1; s <= 3; s++) { // 3 probabilistic paths
        let streamColor = color(baseColor);
        streamColor.setAlpha(20 / s);
        stroke(streamColor);
        beginShape();
        let sx = p.x, sy = p.y;
        for (let j = 0; j < 10; j++) {
           let noiseVal = noise(p.x * 0.01, p.y * 0.01, frameCount * 0.01 + j + s);
           let angle = map(noiseVal, 0, 1, 0, TWO_PI);
           sx -= (p.vx + cos(angle) * 2) * 0.5;
           sy -= (p.vy + sin(angle) * 2) * 0.5;
           vertex(sx, sy);
        }
        endShape();
      }
    ...
    if (spinRibbonMode && p.history.length > 2) {
      ...
      for (let i = 0; i < p.history.length; i++) {
        ...
        let perpX = -h.vy / (sqrt(h.vx*h.vx + h.vy*h.vy) + 0.1);
        let perpY = h.vx / (sqrt(h.vx*h.vx + h.vy*h.vy) + 0.1);
