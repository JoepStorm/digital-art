...
const numTypes = 5;
const numParticles = 400; // Slightly reduced to manage performance with density clouds
const friction = 0.5;
const rMax = 80;
const maxSpeed = 5;
let particles, rules;
...
let cloudMode = true; // Density cloud toggle
...
function draw() {
  background(5, 10); // Darker with trail persistence
  ...
  for (let a of particles) {
    ...
    for (let b of particles) {
      ...
      // Periodic boundary wrapping logic for forces
  ...
  // Draw Probability Clouds (Low alpha gradients to simulate spatial wave functions)
  if (cloudMode) {
    for (let p of particles) {
        let spd = sqrt(p.vx * p.vx + p.vy * p.vy);
        let c = color(colors[p.type]);
        c.setAlpha(map(sin(p.phase), -1, 1, 1, 15));
        fill(c);
        noStroke();
        // The cloud expands based on the current probability phase
    ...
    for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < i + 15; j++) {
            ...
            if (p1.type === p2.type) {
                let dx = p1.x - p2.x;
                let dy = p1.y - p2.y;
                // Prevent lines spanning the width of screen due to screen wrapping
