...
let spinRibbonMode = true; // Toggle for internal spin filaments
...
function draw() {
  ...
  for (let p of particles) {
    ...
    // Draw Spin-State Ribbons: Connecting past positions with a perpendicular spin offset
    if (spinRibbonMode && p.history.length > 2) {
      noFill();
      strokeWeight(0.8);
      let ribbonColor = color(colors[p.type]);
