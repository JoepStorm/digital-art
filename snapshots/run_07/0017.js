...
class Agent {
  constructor() {
    ...
    this.speed = 2.4;
    this.isJet = random() < 0.05; // Kinetic Scribe: Special agents that move faster and carve neon trails
  }
  ...
  updateDirection(timeCycle) {
    ...
    if (distToMouse < chaosRadius) {
      ...
      if (this.isJet) this.speed *= 2.5; // Kinetic Scribe: Jets are much faster
  ...
  updatePosition() {
    ...
    if (this.isJet && random() < 0.6) {
       // Kinetic Scribe: Jets carve vibrant neon blue/cyan riffs into the canvas
    ...
    // Kinetic scribe interaction: jet agents bite deeper into the color buffer
    let intensity = this.isJet ? 140 : 80;
