...
class Agent {
  constructor() {
    ...
    this.isWeaver = random() < 0.02; // Loom Master: Rare agents that seek verticality to "weave" the strata
  }
  ...
  updateDirection(timeCycle) {
    ...
    if (distToMouse < chaosRadius) {
      ...
      if (this.isWeaver) {
          // Loom Master change: Weavers prefer vertical alignment (the "weft") to bridge the horizontal "warp"
  ...
  updatePosition() {
    ...
    if (this.isWeaver) {
       // Loom Master: Weavers leave pale, vertical ghostly silks
