...
// Iteration 15: The Entropy Oracle - Introduced local 'butterfly effect' disturbances where mouse interaction now seeds permanent divergent eddies of chaos within the ordered scaffold.
// Iteration 16: The Gravity Architect - Introduced tectonic stress lines that collapse order into dense horizontal ribbons of matter.
// Iteration 17: The Kinetic Scribe - Introduced high-velocity jet-streams that carve neon-etched rifts through the horizontal strata.
// Iteration 18: The Loom Master - Introduced warp-and-weft interference where agents weave vertical silken threads that bridge the gap between horizontal strata.
// Iteration 19: The Sparksmith - Introduced "electrical discharge" arcs that snap between high-density nodes, bridging chaotic clusters with instantaneous bolts of order.
// Iteration 20: The Frozen Echo - Introduced crystalline "memory ghosts" that freeze agent paths into static cyan geometric shapes when they pass through resonance nodes.
...
function draw() {
  ...
  if (mouseIsPressed) {
    chaosEddies.push({x: mouseX, y: mouseY, r: random(20, 50), life: 255});
    if (chaosEddies.length > 20) chaosEddies.shift();
  }
  ...
  for (let y = 0; y < height; y += 4) {
    for (let x = 0; x < width; x += 4) {
      ...
      if (resonance > 0.8 * (1 - timeCycle)) {
        ...
        if (pixels[idx] < 100 && random() < 0.001) denseNodes.push({x, y});
      }
    }
  }

  if(denseNodes.length > 2 && random() < 0.15) {
      let n1 = random(denseNodes);
      let n2 = random(denseNodes);
      if(dist(n1.x, n1.y, n2.x, n2.y) < 200) {
          scaffold.stroke(100, 200, 255, 180 * (1-timeCycle));
          scaffold.strokeWeight(random(0.5, 2));
          scaffold.beginShape();
          let steps = 5;
          for(let s=0; s<=steps; s++) {
              let tx = lerp(n1.x, n2.x, s/steps) + random(-10, 10);
              let ty = lerp(n1.y, n2.y, s/steps) + random(-10, 10);
              scaffold.vertex(tx, ty);
          }
          scaffold.endShape();
      }
  }
  ...
  for(let i = chaosEddies.length - 1; i >= 0; i--) {
    let e = chaosEddies[i];
    scaffold.stroke(255, 50, 100, e.life * 0.1);
    scaffold.ellipse(e.x, e.y, e.r);
    e.life -= 0.5;
    e.r += 0.2;
    if (e.life <= 0) chaosEddies.splice(i, 1);
  }
...
class Agent {
  constructor() {
    ...
    this.speed = 2.4;
    this.isJet = random() < 0.05; 
    this.isWeaver = random() < 0.02; 
  }
  ...
  updateDirection(timeCycle) {
    ...
    chaosEddies.forEach(e => {
      let d = dist(this.x, this.y, e.x, e.y);
      if (d < e.r * 2) {
        this.dir += (e.life/255) * 0.3 * (1 - d/(e.r*2));
        this.speed *= 1.05;
      }
    });

    let stressLines = abs(sin(this.y * 0.015 + frameCount * 0.005));
    if (stressLines > 0.95 && !this.isWeaver) {
       this.dir = lerp(this.dir, 0, 0.4); 
       this.speed *= 1.8;
    }
    ...
    if (distToMouse < chaosRadius) {
      ...
      if (this.isJet) this.speed *= 2.5; 
      if (this.isWeaver) this.speed *= 1.2;
      ...
      if (this.isWeaver) {
          ...
          if (targetDir === 0 || targetDir === PI) targetDir = HALF_PI;
          this.dir = lerp(this.dir, targetDir, 0.7);
      } else if (gridStrength > orderThreshold || (standingWave > 0.85 && timeCycle > 0.45)) {
        let targetDir = (floor((this.dir + PI/4) / HALF_PI) * HALF_PI);
  ...
  updatePosition() {
    ...
    // The Frozen Echo: Freeze specific path segments into persistent geometric "scars" on resonance
    let freq = 0.02;
    let resonanceNode = (sin(this.x * freq) + cos(this.y * freq)) * 0.5;
    if (resonanceNode > 0.9 && random() < 0.1) {
        scaffold.rectMode(CENTER);
        scaffold.noFill();
        scaffold.stroke(140, 220, 255, 40);
        scaffold.square(this.x, this.y, random(2, 8));
    }

    if (this.isWeaver) {
       scaffold.stroke(220, 230, 255, 30);
       scaffold.strokeWeight(0.5);
       scaffold.line(oldX, oldY, this.x, this.y);
    } else if (this.isJet && random() < 0.6) {
       scaffold.stroke(60, 200, 255, 60);
       scaffold.strokeWeight(1);
       scaffold.line(oldX, oldY, this.x, this.y);
    } else if (this.speed < 2.0 && random() < 0.14) {
       scaffold.strokeWeight(1.5);
       scaffold.line(oldX, oldY, this.x, this.y);
    } else if (abs(cos(this.dir)) > 0.9 && random() < 0.05) {
       scaffold.stroke(100, 100, 255, 15);
       scaffold.line(oldX, oldY, this.x, this.y);
    }
    ...
    let intensity = this.isJet ? 140 : (this.isWeaver ? 40 : 80);
    pixels[i] = max(0, pixels[i] - intensity);
    pixels[i+1] = max(15, pixels[i+1] - (intensity-20));
