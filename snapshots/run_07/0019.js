...
function draw() {
  ...
  const denseNodes = []; // Track locations for sparks
  ...
  for (let y = 0; y < height; y += 4) {
    for (let x = 0; x < width; x += 4) {
      ...
      if (resonance > 0.8 * (1 - timeCycle)) {
        ...
        // Potential spark nodes based on high density (low brightness)
        if (pixels[idx] < 100 && random() < 0.001) denseNodes.push({x, y});
      }
    }
  }

  // Sparksmith logic: Draw electrical arcs between high density points
