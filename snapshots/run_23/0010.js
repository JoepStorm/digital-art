...
// Iteration 9: The Weaver - Added Entropic Decay trails, where particles leave behind ghostly remnants of their past order that slowly dissolve into the chaotic background.
// Iteration 10: The Weaver - Added Geometric Lattice, a dynamic tessellation that emerges when Order is high, connecting particles into a crystalline mesh based on localized density.
const numTypes = 5;
...
let gui;
let entropySlider, resonanceSlider, gravitationalSlider, foldingSlider, bleedSlider, decaySlider, latticeSlider;
let crystalToggle, entanglementToggle, symmetryToggle, gravityToggle, foldingToggle, bleedToggle, decayToggle, latticeToggle;
...
function createInterface() {
  ...
  createDiv('Entropic Decay').parent(controls).style('font-size', '11px').style('margin-top', '8px');
  decaySlider = createSlider(0, 100, 70).parent(controls).style('width', '150px');

  createDiv('Lattice Complexity').parent(controls).style('font-size', '11px').style('margin-top', '8px');
  latticeSlider = createSlider(10, 80, 45).parent(controls).style('width', '150px');
  ...
  let decayDiv = createDiv('').parent(controls).style('margin-top', '4px');
  decayToggle = createCheckbox('Decay Trails', true).parent(decayDiv).style('font-size', '11px');

  let latticeDiv = createDiv('').parent(controls).style('margin-top', '4px');
  latticeToggle = createCheckbox('Geometric Lattice', true).parent(latticeDiv).style('font-size', '11px');
}
...
function draw() {
  ...
  let decayAmt = decaySlider.value() / 100;
  let latRange = latticeSlider.value();
  
  background(0, lerp(25, bleedToggle.checked() ? 8 : 80, entropyAmt));
  ...
  for (let a of particles) {
    ...
    for (let b of particles) {
      ...
      // Basic distance handle for wrapping avoidance
      if (abs(dx) > width/2) continue;
      if (abs(dy) > height/2) continue;
      ...
      if (d2 > 0 && d2 < rMax * rMax) {
        ...
        // Geometric Lattice logic: Connect near neighbors regardless of type when Order is high
        if (latticeToggle.checked() && d < latRange && entropyAmt < 0.4) {
          let opacity = (1 - entropyAmt) * (1 - d/latRange) * 50 * wave;
    ...
    if (decayToggle.checked()) {
      ...
      if (a.history.length > floor(lerp(20, 2, entropyAmt) * decayAmt + 1)) {
        a.history.shift();
      }
    } else {
      a.history = [];
  ...
  for (let p of particles) {
    ...
    if (decayToggle.checked() && p.history.length > 1) {
      noFill();
      strokeWeight(lerp(1.5, 0.2, entropyAmt));
      for (let i = 0; i < p.history.length - 1; i++) {
        let alpha = map(i, 0, p.history.length, 0, 150 * (1 - entropyAmt * 0.5));
        stroke(red(col), green(col), blue(col), alpha);
        line(p.history[i].x, p.history[i].y, p.history[i + 1].x, p.history[i + 1].y);
      }
    }
