...
function draw() {
  // Balanced decay to allow both bright strands and the new "Shadow" erosion to compete
  background(0, singularityActive ? 10 : 18);
...
class Agent {
  constructor() {
    this.isShadow = random() < 0.12; // 12% of agents become entropy-bringers
    this.init();
  }
  ...
  updateDirection() {
    ...
    // Gravitational Pollination - Static seeds influence paths
    for (let s of seeds) {
      let sdx = s.x - this.x;
      let sdy = s.y - this.y;
    ...
    // Shadows move faster to carve through existing structures
    ...
    // Standard Physarum behavior logic: turn towards strongest trail signal
  ...
  sense(dirOffset) {
    ...
    // Sensor sensitivity: Shadows sense trails but follow them to consume them
  ...
  updatePosition() {
    ...
    if (px >= 0 && px < width && py >= 0 && py < height) {
      ...
      if (this.isShadow) {
          // SHADOW EFFECT: Instead of adding color, they subtract it, leaving "hollow" rays
          ...
          if (this.isRadiant) {
              ...
              // Color blending based on vitality and genetic traits
