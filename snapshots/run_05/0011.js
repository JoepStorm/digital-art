...
class Agent {
  ...
  init() {
    ...
    // Radial spawning logic: some close to center, some far
  ...
  updateDirection() {
    ...
    // Gravitational Pollination: Seeds deflect runners
    for (let s of seeds) {
      let dx = s.x - this.x;
      let dy = s.y - this.y;
      let d2 = dx*dx + dy*dy;
  ...
  updatePosition() {
    ...
    if (px >= 0 && px < width && py >= 0 && py < height) {
      ...
      if (!this.isGhost) {
          // TEMPORAL DILATION: Aging is accelerated by environmental density
      ...
      if (this.isGhost) {
          ...
          // MYCELIAL FUSION: When crossing bright patches, agents deposit "calcified" white-blue bridges
          // that serve as permanent highways for future runners.
