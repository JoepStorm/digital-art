...
function step() {
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      let magnetX = (centers[type].x - x) * 0.02;
      let magnetY = (centers[type].y - y) * 0.02;
      ...
      let ox = cos(angle) * (2.5 + metaShift * 10) + ghostShift + magnetX;
      let oy = sin(angle) * (2.5 + metaShift * 10) + ghostShift + magnetY;
      let px = x + pad + ox;
      ...
      // Sculptor's Void: If density is extreme, reverse the logic to carve hollows
      let repulsion = (localDensity > 0.95) ? -0.2 : 0;
      ...
      for (let rule of activeRules) {
        ...
        if (avg >= lo && avg <= hi) { target = s + repulsion; break; }
      }
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      for (let py = 0; py < res; py++) {
        let offsetBase = (rowStart + py * width + xOffset) * 4;
        for (let px = 0; px < res; px++) {
          let pi = offsetBase + px * 4;
          pixels[pi] = r;
          pixels[pi + 1] = g;
          pixels[pi + 2] = b;
          pixels[pi + 3] = 255;
