...
function step() {
  buildSAT();
  let currentFade = 0.05 + 0.1 * (0.5 + 0.5 * sin(frameCount * 0.02)); 
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      // The Rift: Calculate a spatial offset based on a vortex field
      // This warps where the cell "looks" for its neighbors, creating swirling structures
      let angle = noise(x * 0.012, y * 0.012, frameCount * 0.005) * TWO_PI * 4;
      let ox = cos(angle) * 1.5;
      let oy = sin(angle) * 1.5;
      let px = x + pad + ox;
      ...
      for (let [ri, lo, hi, s] of activeRules) {
        // Linear interpolation for sub-pixel sampling would be slow, 
        // using floor for speed while maintaining the warped effect.
        let avg = ringAvg(floor(px), floor(transformedPy), ri);
        if (avg >= lo && avg <= hi) { target = s; break; }
      }
      
      next[idx] = state + (target - state) * currentFade;
...
function draw() {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      else if (type === 1) { r = v * 255; g = v * 40; b = v * 200; }
      else { r = v * 255; g = v * 220; b = v * 40; }
