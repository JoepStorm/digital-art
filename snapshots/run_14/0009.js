...
let prevGrid; // Stores the state from the previous frame for velocity-based effects
...
function step() {
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      // Compute temporal velocity to influence spatial sampling (Ghosting effect)
      let velocity = state - prevGrid[idx];
      let ghostShift = velocity * 12.0;

      let d = dist(x, y, midX, midY) / maxDist;
      let thresholdShift = d * 0.08;
      ...
      prevGrid[idx] = state; // Store current for next frame velocity
