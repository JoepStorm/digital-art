...
function boxSum(px, py, r) {
  ...
  // Fast SAT lookup for O(1) rectangular sampling
...
function step() {
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      // The Architect: Rectilinear quantization of the noise field
      // By snapping the angle to 90-degree increments, textures gain an architectural, circuit-like quality
      let n = noise(x * 0.015, y * 0.015, frameCount * 0.005);
      let angle = floor(n * 4) * HALF_PI;
      let ox = cos(angle) * 2.5;
      let oy = sin(angle) * 2.5;
      let px = x + pad + ox;
      ...
      for (let rule of activeRules) {
        let ri = rule[0], lo = rule[1], hi = rule[2], s = rule[3];
      ...
      // Stochastic migration of cell identities
      if (state < 0.1 && random() < 0.003) {
...
function draw() {
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      // Visualizing species through RGB space with high contrast offsets
      let r = 0, g = 0, b = 0;
