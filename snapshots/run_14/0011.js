...
function step() {
  ...
  // Calculate species mass centers for magnetic drift
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      // Magnetic drift towards own species center
      ...
      let ox = cos(angle) * 2.5 + ghostShift + magnetX;
      let oy = sin(angle) * 2.5 + ghostShift + magnetY;
      let px = x + pad + ox;
