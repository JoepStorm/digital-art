...
function setup() {
  ...
  // Initialize tendrils as autonomous agents that react to the grid
  for(let i=0; i<12; i++) {
    tendrils.push({
      x: random(cols), 
      y: random(rows), 
...
function step() {
  ...
  // Mycelial Tendril Logic: Agents that deposit charge/state as they navigate gradients
  tendrils.forEach(t => {
    let ix = floor(t.x + cols) % cols;
    let iy = floor(t.y + rows) % rows;
    let idx = ix + iy * cols;
    // Follow the flow field but also seek high-density areas
    t.vx += (flowX[idx] * 0.1 + (random(-0.1, 0.1)));
    t.vy += (flowY[idx] * 0.1 + (random(-0.1, 0.1)));
    t.vx *= 0.95; t.vy *= 0.95;
    t.x += t.vx; t.y += t.vy;
    grid[idx] = min(1, grid[idx] + 0.1); // Growth in their wake
    charge[idx] = min(2, charge[idx] + 1.5); // Leave a bright trail
  });
...
function draw() {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      // Bioluminescent filament boost: intensified by 'charge' to simulate active energy pathways
