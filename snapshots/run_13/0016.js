...
function step() {
  ...
  // Accretion modulation: select history based on distance to the moving singularity 
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      // Temporal accretion logic: bits closer to the singularity draw from further back in time
      ...
      next[idx] = (stateSum + (stipple - stateSum) * fade) * 0.995;
    }
  }
  history.pop();
...
function draw() {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      let dy = y - singY;
      let d2 = dx*dx + dy*dy;
      // Event Horizon Rendering: Intentionally distorting the lookup to create a 'swirl' near the singularity
      let lensFactor = 700 / (d2 + 150);
      let swirl = 0.8 * exp(-d2 * 0.0001);
      ...
      let b = val * (127 + 127 * sin(huePulse * 0.7 - radialAngle + (rDist*0.02) - chg));
      
      let r8 = constrain(r + grad * 12 + chg * 15, 0, 255);
      let g8 = constrain(g + grad * 15 + (1.0/lensFactor) * 2, 0, 255);
      let b8 = constrain(b + grad * 20 + chg * 30, 0, 255);
