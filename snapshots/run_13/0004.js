...
function step() {
  ...
  // The Singularity: A wandering point moving in a complex Lissajous pattern
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      // Calculate distortion based on distance to the wandering singularity
      let dSing = dist(x, y, singX, singY);
      let singularityInfluence = 1.8 * exp(-dSing * 0.1); 
      
      let distFromCenter = dist(x, y, cols/2, rows/2) / (cols/2);
      // Combine global bloom warp with the localized singularity punch
      ...
      for (let i = 0; i < rules.length; i++) {
        ...
        // Singularity also drifts the sensitivity thresholds locally
