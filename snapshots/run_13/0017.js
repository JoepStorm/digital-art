...
function draw() {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      // Interference pattern: adds high-frequency luminescent "scalloping" based on global time and local coordinate
      ...
      let lum = (interference > 0.1) ? map(sin(t*10 + lx*0.1), -1, 1, 0, 80) : 0;
      let r8 = constrain(r + grad * 12 + chg * 15 + lum, 0, 255);
      let g8 = constrain(g + grad * 15 + (1.0/lensFactor) * 2 + lum * 0.5, 0, 255);
      let b8 = constrain(b + grad * 20 + chg * 30 + lum * 2, 0, 255);
