...
function draw() {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      // Spatial interference: color varies based on grid position and value
      // This creates a "prism" effect where the simulation looks like a refractive surface
      let angle = atan2(y - rows/2, x - cols/2) + t;
      let distFactor = dist(x, y, cols/2, rows/2) * 0.05;
      
      let r = v * (127 + 127 * sin(t + distFactor + angle));
      let g = v * (127 + 127 * sin(t * 1.2 + distFactor * 0.7 - angle));
      let b = v * (127 + 127 * sin(t * 0.8 + distFactor * 1.3));
      
      for (let py = 0; py < res; py++) {
        let row = (y * res + py) * width;
        let baseIndex = (row + x * res) * 4;
