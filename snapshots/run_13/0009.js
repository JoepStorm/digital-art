...
function step() {
  ...
  // Update velocity field based on local density gradients (simulated convection)
...
function draw() {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      let r = val * (140 + 115 * sin(t + radialAngle + grad + flowX[idx]*0.1));
      let g = val * (140 + 115 * sin(t * 0.9 + radialAngle + PI/2 - grad * 0.5));
      let b = val * (140 + 115 * sin(t * 1.1 - radialAngle + PI + drift + flowY[idx]*0.1));
      
      let burst = pow(val, 0.5) * 255;
