...
let history = []; // Keep track of past states for feedback loops
...
function setup() {
  ...
  for(let i=0; i<30; i++) history.push(new Float32Array(cols * rows).fill(0));
}
...
function step() {
  ...
  // Feedback depth modulation
  let historyIdx = floor(map(sin(frameCount * 0.02), -1, 1, 0, history.length - 1));
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      // Synaptic Feedback: The pixel's state is influenced by its own past resonance
      let echo = prevGrid[idx];
      let stateMod = state * 0.7 + echo * 0.3;
      ...
      next[idx] = stateMod + (target - stateMod) * fade;
    }
  }
...
function draw() {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      let gx = (grid[((x+1)%cols) + y*cols] - grid[((x-1+cols)%cols) + y*cols]) * 15;
      let gy = (grid[x + ((y+1)%rows)*cols] - grid[x + ((y-1+rows)%rows)*cols]) * 15;
      
      let dx = x - cols/2;
      let dy = y - rows/2;
      let distSq = dx*dx + dy*dy;
      let angle = atan2(dy, dx) + t;
      let distFactor = sqrt(distSq) * 0.05;
      let refract = (gx + gy) * 0.5;
      
      // Color dynamics enhanced by intensity mapping
      let r = v * (127 + 127 * sin(t + distFactor + angle + refract));
      let g = v * (127 + 127 * sin(t * 1.2 + distFactor * 0.7 - angle + PI/2 + refract * 0.8));
      let b = v * (127 + 127 * sin(t * 0.8 + distFactor * 1.3 + PI + refract * 1.2));
