...
function boxSum(px, py, r) {
  let w = pCols + 1;
  // Constraint: clamp radii to available padding to prevent out-of-bounds SAT access
  r = Math.min(r, pad - 1);
...
function ringAvg(px, py, ri, warp) {
  let [r1, r2] = rings[ri];
  // Apply the spatial warp to the neighborhood radius
...
function step() {
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      // Dynamic Warp: Neighbours radii change based on local density and radial distance
      let distFromCenter = dist(x, y, cols/2, rows/2) / (cols/2);
      let warp = 1.0 + 0.4 * sin(frameCount * 0.02 + distFromCenter * PI);
...
function draw() {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      // Shift hue dynamically based on simulation state (v)
      ...
      // Fast manual pixel setting for performance
