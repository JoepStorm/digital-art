...
let history = []; 
let flowX, flowY; 
let charge; // New persistence layer for "ionizing" trails
...
function step() {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      // Accumulate 'charge' based on local movement intensity
      charge[idx] = lerp(charge[idx], (abs(flowX[idx]) + abs(flowY[idx])), 0.05);
    }
  }
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      next[idx] = (stateSum + (target - stateSum) * fade) * 0.994;
    }
  }
  history.pop();
...
function draw() {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      let idx = lx + ly * cols;
      let val = grid[idx];
      let chg = charge[idx] * 0.8; // Use accumulated motion charge for visual intensity
      ...
      // Colors now shift more aggressively based on local kinetic charge (chg)
      let r = val * (127 + 127 * sin(t + radialAngle + grad + flowX[idx] * 0.2 + chg));
      let g = val * (127 + 127 * sin(t * 1.2 + radialAngle * 2 - grad * 0.4 + chg * 0.5));
      let b = val * (127 + 127 * sin(t * 0.8 - radialAngle + drift + flowY[idx] * 0.2 - chg));
      
      let burst = pow(val, 0.45) * 255;
      // High-charge areas emit a "white-hot" plasma effect
      let r8 = constrain(r + grad * 6 + chg * 10, 0, 255);
      let g8 = constrain(g + grad * 10 + chg * 20, 0, 255);
      let b8 = constrain(b + grad * 14 + chg * 30, 0, 255);
